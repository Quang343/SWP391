...
public class SellerApplicationAdminController {
    ...
    private BigDecimal getAmount(SellerApplication app){
...
//        System.out.println("Contract months: " + contractMonths);
        ...
        return originalTotal.subtract(discountedAmount).setScale(2, RoundingMode.HALF_UP);
    }
    ...
    public String reject(@PathVariable("id") Long saId, RedirectAttributes redirectAttributes) {
        ...
        if (optionalSa.isPresent()) {
            ...
            if (sa.getStatus() == SellerApplicationStatus.PENDING) {
                ...
                Optional<Wallets> wallets = walletsRepository.findByUser(sa.getUser());

                if(wallets.isPresent()){
                    WalletsResponse walletsResponse = walletsUsersService.getBalanceWallet(sa.getUser().getUserId());
                    walletsResponse.setBalance(walletsResponse.getBalance().add(getAmount(sa)));
                    wallets.get().setBalance(walletsResponse.getBalance());
                    walletsRepository.save(wallets.get());
                }
                ...
                MailBody mailBody = MailBody.builder()
                        ...
                        .text("Xin chào " + sa.getUser().getFullName() + ",\n\n"
                                ...
                                + "Rất tiếc, sau khi xem xét, hồ sơ của bạn chưa đáp ứng yêu cầu và đã bị từ chối. Bạn sẽ được hoàn trả lại toàn bộ số tiền.\n\n"
