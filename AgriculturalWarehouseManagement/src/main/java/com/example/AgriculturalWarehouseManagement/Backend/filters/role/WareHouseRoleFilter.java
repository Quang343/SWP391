...
@WebFilter("/warehouse/*") // Áp dụng filter cho tất cả các trang
public class WareHouseRoleFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        UserResponse account = (UserResponse) session.getAttribute("accountWarehouse");

        // Kiểm tra nếu không có tài khoản hoặc tài khoản không có quyền ADMIN
        if (account == null || !account.getRole().getRoleName().equalsIgnoreCase("WAREHOUSE")) {
