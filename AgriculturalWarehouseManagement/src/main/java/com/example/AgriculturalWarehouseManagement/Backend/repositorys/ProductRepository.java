...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    @Query(value = """
                                    ...
                                    JOIN (
                                     ...
                                     SELECT  p.ProductID               AS productId,
                                             ...
                                             cd.Weight                 AS productWeight,
                                             p.Description             AS productDescription
                                     FROM       Product         p
                                     JOIN       cheapest_detail cd ON cd.ProductID = p.ProductID
    ...
    @Query(value = """
                                   WITH first_img AS (          -- Lấy 1 ảnh đầu tiên của mỗi sản phẩm
                                       SELECT g.ProductID, g.ImageURL
                                       FROM Gallery g
                                       JOIN (
                                           SELECT ProductID, MIN(GalleryID) AS min_id
                                   ...
                                   avg_rating AS (              -- Tính điểm trung bình & số lượt đánh giá
                                       SELECT  ProductID,
                                               ROUND(AVG(Rating)) AS avgRating,
                                               COUNT(*)           AS ratingCount
                                       FROM CommentProduct
                                   ...
                                   cheapest_detail AS (         -- ★ mỗi ProductID chỉ giữ dòng rẻ nhất
                                       SELECT *
                                       FROM (
                                           SELECT  pd.*,
                                                   ROW_NUMBER() OVER (PARTITION BY pd.ProductID
                                   ...
                                   top9 AS (                     -- ★ 9 sản phẩm “đắt” nhất dựa trên giá rẻ nhất của chúng
                                       SELECT ProductID
                                       FROM cheapest_detail
                                       ORDER BY Price DESC
                                       LIMIT 9
                                   )
                                   SELECT  p.ProductID                        AS productId,
                                   ...
                                   LEFT JOIN avg_rating     ar  ON ar.ProductID = p.ProductID
                                   WHERE     p.Status = 'ACTIVE'
                                   ORDER BY  cd.Price DESC;  -- đảm bảo kết quả vẫn theo giá giảm dần
            """, nativeQuery = true)
