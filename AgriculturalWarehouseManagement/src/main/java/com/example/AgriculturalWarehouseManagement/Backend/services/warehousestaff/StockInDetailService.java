...
public class StockInDetailService {
    ...
    public BigDecimal calculateTotalSpent() {
        List<StockInDetail> allDetails = stockInDetailRepository.findAll();

        BigDecimal total = BigDecimal.ZERO;

        for (StockInDetail detail : allDetails) {
            Integer unitPrice = detail.getUnitPrice();
    ...
    public int countBatchesInStockIn(Long stockInId) {
        StockIn stockIn = stockInRepository.findById(stockInId.intValue())
                .orElseThrow(() -> new RuntimeException("Không tìm thấy đơn nhập với ID: " + stockInId));
